...
public class CartServiceImpl implements CartService{
	...
	public String submitOrder(Order order, Model model, HttpSession session) {
		order.setcustomer_id(MyUtil.getUser(session).getId());
		//生成订单
	...
	public String updateUpwd(HttpSession session, String pwd) {
		Integer uid = MyUtil.getUser(session).getId();
		cartRepository.updateUpwd(uid, MD5Util.MD5(pwd));
